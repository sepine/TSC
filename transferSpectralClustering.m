...
[nt d] = size(Xt); %1250*5000
ns = size(Xs,1);

% n = nt + ns;

A1 = Xt - min(min(Xt));%1250*5000
A2 = Xs - min(min(Xs));
% A1(A1>0) = 1;A2(A2>0) = 1; 
[A1] = normalize_matrix(A1);    %对应函数在下面 1250*5000
...
[W11] = normalize_matrix(W11); %1250*1250
[W22] = normalize_matrix(W22);
W11 = (W11 + W11')/2; %对称
W22 = (W22 + W22')/2;


[F1,D1] = eigs(W11,k,'LA'); %返回k个最大特征值 'la'表示最大特征值   D1按降序排列
                            %D1为k个最大特征值对角阵，F1的列向量为对应特征向量
[D1,index] = sort(diag(D1),'descend');
F1 = F1(:,index);   %特征值最大的特征向量排第一列 1250*k
% F1 = randn(size(F1));
...
[F1,F2,F3] = init_F3(F1,F2,A1,A2,k); %F3 5000*k
% F3 = randn(size(F3));
...
if r > 0
    while true
        if rem(iter,100) == 0  %rem(x,y):求整除x/y的余数
...
for i = 1:nt
    F1(i,:) = F1(i,:)/norm(F1(i,:));  %得到Z（1）收敛后的下一步 1250*k
end

for i = 1:ns
    F2(i,:) = F2(i,:)/norm(F2(i,:)); %得到Z（2）收敛后的下一步 1250*k
end

end

function [f,f1,f2,f3,f4] = compute_obj(W11,W22,F1,F2,F3,A1,A2,r)  %公式13
f1 = trace(F1'*W11*F1); %b = trace(A) 是矩阵 A 的对角线元素之和。
f2 = trace(F2'*W22*F2);
f3 = r*trace(F1'*A1*F3);
f4 = r*trace(F2'*A2*F3);
f = f1 + f2 + f3 + f4;
...
end   %assert函数用来判断一个expression是否成立，如不成立则报错


function [A] = normalize_matrix(A)
[nt ns] = size(A);

D1 = sum(A,2);% sum of row nt*1
D2 = sum(A,1);% 1*ns 列求和
D1 = sqrt(D1);
D2 = sqrt(D2);
...
[A] = normalize_matrix([A1;A2]); %2500*5000
% [U S V] = svds(A,l+1);
% n = size(A,2);
% F = zeros(n,k);

% [U S V] = svd(full(A),'econ');
[U S V] = svds(A,k); %[U,S,V]=svds(M,k);则U(m,k),S(k,k),V(n,k).此时svds返回的就是最大的k个特征值及其对应的特征行向量和列向量。
[s index] = sort(diag(S),'descend');
F3 = V(:,index); %5000*k 特征值越大排越前
% sum(s)
for i = 1:k
    F3(:,i) = F3(:,i)/norm(F3(:,i));
    t1 = F1(:,i)'*A1*F3(:,i);
